...
public partial class CsharpTest : Node
{
	...
	Godot.Collections.Array available_move = new Godot.Collections.Array();
...
// <------------------------ Simple stuff starts ------------------------>
	...
	public string get_cell(int cell){
		return new string(chessboard[cell], 1);
	}
	
	public void set_cell(int cell, string val){
		chessboard[cell] = val[0];
	}
	...
	public CsharpTest deep_copy(){
		CsharpTest tmp = new_object(init_state);
...
// <------------------------ Handling move starts ------------------------>
	...
	public void en_passant(int cell1, int cell2){
		...
		Vector2 coord1 = Utility.int_to_cell_vector(cell1);
		Vector2 coord2 = Utility.int_to_cell_vector(cell2);
		Vector2 coord3 = new Vector2(coord1.X, coord2.Y);
		
		int cell3 = Utility.vector_to_cell_index(coord3);
...
// <------------------------ Move Validator Start ------------------------>
	...
	public bool in_check(bool current_side){
		...
		for(int i = 0; i < 64; ++i) if (chessboard[i] == cur) 
			return check_tile_attacked(i, !current_side);
		return false;
	}
	...
	private List<ChessMove> generate_move_type_from_cell(int cell1){
		...
		if (cur == 'k' || cur == 'n'){
			...
			for(int i = 0; i < d_coord.Count; ++i){
				...
				if (Math.Min(coord.X, coord.Y) >= 0 && Math.Max(coord.X, coord.Y) < 8){
					if ((chessboard[cell2] == '.') || (char.IsUpper(chessboard[cell2]) != char.IsUpper(chessboard[cell1])))
						move_list.Add(new ChessMove(cell1, cell2, (int)MOVE.Castle));
				}
			}
		}
		...
		for(int i = 0; i < 64; ++i){
			if ((chessboard[i] != '.') && (char.IsUpper(chessboard[i]) == white_move)){
				...
				foreach (var k in current_list){
					...
					if (k.move_type == (int)MOVE.Normal || k.move_type == (int)MOVE.Promote)
						tmp_board.normal_move(i, k.des);
					if (k.move_type == (int)MOVE.Castle)
						tmp_board.castle(i, k.des);
					if (k.move_type == (int)MOVE.EnPassant)
						tmp_board.en_passant(i, k.des);
					//if (tmp_board.in_check(white_move))
						//continue;
					if (k.move_type != (int)MOVE.Promote){
						available_move.Add(k);
					}
					else{
						for(int j = 0; j < 4; ++j)
							available_move.Add(new ChessMove(k.start, k.des, k.move_type, j));
					}
				}
			}
		}
...
// <------------------------ Game State Messing Start ------------------------>
	...
	public string stupid_draw_check(){
		...
		if (remaining_pieces.Count <= 1){
			...
			else if (remaining_pieces[0] == 'b' || remaining_pieces[1] == 'n')
